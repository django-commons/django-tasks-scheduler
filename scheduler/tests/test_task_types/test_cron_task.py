...
class TestCronTask(BaseTestCases.TestBaseTask):
    ...
    def test_cron_task_enqueuing_jobs(self):
        ...
        entry = JobModel.get(first_run_id, connection=queue.connection)
        queue.run_sync(entry)
        self.assertEqual(20, queue.queued_job_registry.count(queue.connection))
        self.assertEqual(prev_finished + 1, queue.finished_job_registry.count(queue.connection))
        worker = create_worker("default", fork_job_execution=False, burst=True)
        worker.work()
        self.assertEqual(prev_finished + 21, queue.finished_job_registry.count(queue.connection))
