...
def _find_job(job_name: str) -> Tuple[Optional[Queue], Optional[JobModel]]:
    ...
    for queue_name in queue_names:
        ...
        try:
            ...
            logger.debug(f"Queue {queue_name} added to bad configuration")
            pass
    return None, None
...
def job_detail(request: HttpRequest, job_name: str) -> HttpResponse:
    ...
    if job is None:
        return HttpResponseBadRequest(f"Job {escape(job_name)} does not exist, maybe its TTL has passed")
    try:
        job.func_name
        data_is_valid = True
    except Exception:
...
def actions(request: HttpRequest, queue_name: str) -> HttpResponse:
    ...
    if action == "delete":
        ...
        for job in jobs:
            if job is None:
                continue
            # Remove job name from queue and delete the actual job
