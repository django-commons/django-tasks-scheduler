...
class BaseModel:
    ...
    def serialize(self) -> Dict[str, str]:
        data = dataclasses.asdict(
            self, dict_factory=lambda fields: {key: value for (key, value) in fields if value is not None}
        )
        for k in data:
...
class HashModel(BaseModel):
    ...
    def get_many(cls, names: Sequence[str], connection: ConnectionType) -> List[Self]:
        ...
        return [(cls.deserialize(decode_dict(v, cls._byte_keys)) if v else None) for v in values]
