...
class WorkerScheduler:
    ...
    def _acquire_locks(self) -> Set[str]:
        ...
        if self.pid is None:
            logger.info(f"[Scheduler {self.worker_name}] PID is None, not acquiring locks")
            return set()
        logger.debug(f"""[Scheduler {self.worker_name}/{self.pid}] Trying to acquire locks for {", ".join(self._queue_names)}""")
    ...
    def heartbeat(self):
        ...
        logger.debug(f"[Scheduler {self.worker_name}/{self.pid}] Scheduler sending heartbeat to {lock_keys}")
    ...
    def stop(self):
        logger.info(f"[Scheduler {self.worker_name}/{self.pid}] Stopping scheduler, releasing locks for {', '.join(self._locks.keys())}...")
        self.release_locks()
        self._status = SchedulerStatus.STOPPED
    ...
    def work(self) -> None:
        logger.info(f"""[Scheduler {self.worker_name}/{self.pid}] Scheduler for {", ".join(self._queue_names)} started""")
        django.setup()
