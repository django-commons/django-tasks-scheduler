...
import scheduler
from scheduler.broker_types import (ConnectionType, TimeoutErrorTypes, ConnectionErrorTypes, WatchErrorTypes,
                                    ResponseErrorTypes)
...
class Worker:
    ...
    def _install_signal_handlers(self) -> None:
        ...
        if threading.current_thread() is not threading.main_thread():
            logger.info("Running in a thread, skipping signal handlers installation")
            return
    ...
    def register_birth(self):
        ...
        logger.info(f"Registered birth of worker: {self._model}")
    ...
    def request_stop(self, signum: int, frame: Optional[FrameType]) -> None:
        ...
        logger.info(f"Worker {self.name} [PID {self._model.pid}]: warm shut down requested")
        self._shutdown()
    ...
    def stop_scheduler(self):
        ...
        if not self.scheduler.pid:
            return
        logger.info(f"Worker {self.name}: Stopping scheduler process {self.scheduler.pid}")
        try:
