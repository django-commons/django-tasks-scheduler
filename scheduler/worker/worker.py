...
from typing import Callable, List, Optional, Tuple, Any, Iterable, Self
...
class Worker:
    ...
    def work(
        ...
        try:
            while True:
                self._model = WorkerModel.get(self._model.name, connection=self.connection)
    ...
    def handle_job_failure(self, job: JobModel, queue: Queue, exc_string=""):
        ...
        with self.connection.pipeline() as pipeline:
            retry = job.retries_left and job.retries_left > 0 and not new_job_status != JobStatus.STOPPED

            if new_job_status == JobStatus.STOPPED:
                logger.debug(f"[Worker {self.name}/{self._pid}]: Job was stopped, setting status to STOPPED")
            elif not retry:  # Requeue/reschedule if retry is configured, otherwise
                logger.debug(f"[Worker {self.name}/{self._pid}]: Job has no retries, setting status to FAILED")
    ...
    def bootstrap(self, with_command_listener: bool = True):
        ...
        logger.info(f"""*** Worker {self.name} Listening to queues {", ".join(qnames)}...""")
    ...
    def run_maintenance_tasks(self):
        ...
        1. Check if scheduler should be started. This check should not be run on first run since worker.work() already
           calls `scheduler.enqueue_scheduled_jobs()` on startup.
    ...
    def request_force_stop(self, signum: int, frame: Optional[FrameType]):
        ...
        if self._model.shutdown_requested_date is not None and (
                utcnow() - self._model.shutdown_requested_date
        ) < timedelta(seconds=1):
            logger.debug(
    ...
    def stop_scheduler(self):
        ...
        """
        if not self.scheduler.pid:
            return
        logger.info(f"[Worker {self.name}/{self._pid}]: Stopping scheduler process {self.scheduler.pid}")
        try:
    ...
    def fork_job_execution_process(self, job: JobModel, queue: Queue) -> None:
        ...
        if child_pid == 0:  # Child process/Job executor process to run the job
            os.setsid()
            self._pid = os.getpid()
            self._model.set_field("job_execution_process_pid", self._pid, self.connection)
    ...
    def handle_exception(self, job: JobModel, *exc_info):
        ...
        for handler in self._exc_handlers:
            logger.debug(f"[Worker {self.name}/{self._pid}]: Invoking exception handler {handler}")
