...
class KillWorkerCommand(WorkerCommand):
    ...
    def process_command(self, connection: ConnectionType) -> None:
        ...
        try:
            Worker.from_model(worker_model).request_stop(signal.SIGKILL, None)
            logger.info(f"Killed worker main process pid {self.worker_pid}")
        except OSError as e:
            if e.errno == errno.ESRCH:
                logger.debug(
                    f"Worker main process for {self.worker_name}:{self.worker_pid} already dead")  # "No such process" is fine with us
            else:
                raise
