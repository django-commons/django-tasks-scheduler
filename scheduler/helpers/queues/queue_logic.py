...
class Queue:
    ...
    def get_all_jobs(self) -> List[JobModel]:
        job_ids = self.get_all_job_names()
    ...
    def job_handle_failure(self, status: JobStatus, job: JobModel, exc_string: str, connection: ConnectionType):
        ...
        Result.create(
            connection, job.name, ResultType.FAILED, SCHEDULER_CONFIG.DEFAULT_FAILURE_TTL, exc_string=exc_string
        )
    ...
    def enqueue_at(self, when: datetime, f, *args, **kwargs) -> JobModel:
        ...
        """
        job_model = JobModel.create(
            connection=self.connection, queue_name=self.name, func=f, status=JobStatus.SCHEDULED, *args, **kwargs
        )
